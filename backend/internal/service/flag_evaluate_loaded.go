...
// EvaluateLoadedFlags evaluates flags that were already loaded for exactly
// one project/environment. It is used by the data plane, which intentionally
// has no database connection.
func EvaluateLoadedFlags(flags []model.FeatureFlag, key string, evalCtx map[string]any) EvaluationResult {
...
// EvaluateAllLoadedFlags evaluates every flag in the supplied loaded scope.
func EvaluateAllLoadedFlags(flags []model.FeatureFlag, evalCtx map[string]any) []EvaluationResult {
