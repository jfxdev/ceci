...
func TestOIDCService_BeginCreatesSignedPKCETransaction(t *testing.T) {
	svc := &OIDCService{signKey: []byte("test-signing-key"), oauth: oauth2.Config{ClientID: "client", RedirectURL: "http://localhost/callback", Endpoint: oauth2.Endpoint{AuthURL: "https://idp.example.com/authorize"}}}
...
func TestOIDCService_RejectsTamperedOrExpiredTransaction(t *testing.T) {
	svc := &OIDCService{signKey: []byte("test-signing-key")}
