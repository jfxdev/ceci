// Package project holds the project domain logic (creation, membership,
// role resolution), split out from internal/service so it can be maintained
// and tested independently of the rest of the service layer. Models
// (model.Project/ProjectMember) and persistence (repository.ProjectRepository
// and friends) stay in their existing packages — only the business logic
// moved here.
package project
...
func NewService(projects repository.ProjectRepository, users repository.UserRepository, envs repository.EnvironmentRepository, templates repository.EnvironmentTemplateRepository, groups ...repository.AccessGroupRepository) *Service {
	var accessGroups repository.AccessGroupRepository
	if len(groups) > 0 {
		accessGroups = groups[0]
	}
	return &Service{projects: projects, users: users, envs: envs, templates: templates, groups: accessGroups}
}
...
func (s *Service) Create(ctx context.Context, creatorID uuid.UUID, name, slug string, environmentTemplateKeys []string) (*model.Project, error) {
...
func (s *Service) Update(ctx context.Context, id uuid.UUID, name string) (*model.Project, error) {
...
func (s *Service) ListForUser(ctx context.Context, userID uuid.UUID) ([]model.Project, error) {
...
func (s *Service) RoleOf(ctx context.Context, projectID, userID uuid.UUID) (constants.ProjectRole, error) {
...
func (s *Service) ListMembers(ctx context.Context, projectID uuid.UUID) ([]repository.MemberWithUser, error) {
	return s.projects.ListMembers(ctx, projectID)
}

func (s *Service) AddMember(ctx context.Context, projectID uuid.UUID, email string, role constants.ProjectRole) error {
...
func (s *Service) UpdateMemberRole(ctx context.Context, projectID, userID uuid.UUID, role constants.ProjectRole) error {
