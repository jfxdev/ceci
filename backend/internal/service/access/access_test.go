...
func newAccessGroupServiceForTest(t *testing.T) (*Service, repository.AccessGroupRepository, repository.UserRepository, *gorm.DB) {
	t.Helper()
	db := newAccessGroupTestDB(t)
	groups := repository.NewAccessGroupRepository(db)
	users := repository.NewUserRepository(db)
	return NewService(groups, users), groups, users, db
}
...
func TestAccessGroupService_AddMemberValidatesUserAndDuplicate(t *testing.T) {
	...
	assert.ErrorIs(t, err, project.ErrUserNotFound)
...
func TestAccessGroupService_ProjectGrantsExcludeOwnerAndPreventDuplicates(t *testing.T) {
	...
	require.NoError(t, err)
	proj := &model.Project{Name: "Payments", Slug: "payments"}
	require.NoError(t, db.Create(proj).Error)

	err = svc.GrantProject(ctx, proj.ID, group.ID, constants.RoleOwner)
	assert.ErrorIs(t, err, ErrOwnerRole)
	require.NoError(t, svc.GrantProject(ctx, proj.ID, group.ID, constants.RoleViewer))
	err = svc.GrantProject(ctx, proj.ID, group.ID, constants.RoleViewer)
	assert.ErrorIs(t, err, ErrGrantExists)
	require.NoError(t, svc.UpdateProjectGrant(ctx, proj.ID, group.ID, constants.RoleAdmin))
	grants, err := svc.ListProjectGrants(ctx, proj.ID)
	require.NoError(t, err)
	require.Len(t, grants, 1)
	assert.Equal(t, constants.RoleAdmin, grants[0].Role)
	require.NoError(t, svc.RevokeProjectGrant(ctx, proj.ID, group.ID))
	_, err = groups.FindProjectGrant(ctx, proj.ID, group.ID)
	assert.ErrorIs(t, err, repository.ErrNotFound)
}
...
func TestProjectService_UsesHighestDirectOrGroupRole(t *testing.T) {
	...
	projectService := project.NewService(projects, users, nil, nil, groups)
	ctx := context.Background()
	proj := &model.Project{Name: "Payments", Slug: "payments"}
	...
	require.NoError(t, projects.AddMember(ctx, &model.ProjectMember{ProjectID: proj.ID, UserID: user.ID, Role: constants.RoleViewer}))
	require.NoError(t, groups.AddMember(ctx, &model.AccessGroupMember{AccessGroupID: group.ID, UserID: user.ID}))
	require.NoError(t, groups.GrantProject(ctx, &model.ProjectAccessGroup{ProjectID: proj.ID, AccessGroupID: group.ID, Role: constants.RoleEditor}))

	role, err := projectService.RoleOf(ctx, proj.ID, user.ID)
	require.NoError(t, err)
	assert.Equal(t, constants.RoleEditor, role)
	require.NoError(t, projects.UpdateMemberRole(ctx, proj.ID, user.ID, constants.RoleOwner))
	role, err = projectService.RoleOf(ctx, proj.ID, user.ID)
	require.NoError(t, err)
	...
	assert.Equal(t, proj.ID, listed[0].ID)
}

func TestAccessGroupService_DeleteMissingGroup(t *testing.T) {
	svc, _, _, _ := newAccessGroupServiceForTest(t)
	assert.ErrorIs(t, svc.Delete(context.Background(), uuid.New()), ErrNotFound)
}
