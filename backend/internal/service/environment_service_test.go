...
func TestEnvironmentService_CreateDuplicateKey(t *testing.T) {
	...
	assert.ErrorIs(t, err, ErrEnvironmentKeyTaken)
}

func TestEnvironmentService_UpdateNotFound(t *testing.T) {
	svc := NewEnvironmentService(newFakeEnvRepository())
...
func TestEnvironmentService_Delete_DefaultEnvironmentProtected(t *testing.T) {
	...
	assert.ErrorIs(t, err, ErrDefaultEnvironment, "the default env must never be deletable, even with other environments present")
}
