...
func TestMaintenanceService_ValidatesAndPreservesCustomMessage(t *testing.T) {
	...
	_, err := service.Update(context.Background(), true, "   ", adminID)
	assert.ErrorIs(t, err, ErrMaintenanceMessageRequired)

	status, err := service.Update(context.Background(), true, "  Updating the database.  ", adminID)
	...
	status, err = service.Update(context.Background(), false, "ignored", adminID)
	require.NoError(t, err)
